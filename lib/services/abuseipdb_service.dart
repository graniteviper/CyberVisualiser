...
class AbuseIpDbService {
  ...
  Future<AbuseIpReport> fetchIpReport({
    ...
    if (apiKey.isEmpty) {
      throw Exception('AbuseIPDB API Key is not loaded. Please verify .env configuration.');
    }
    ...
    if (response.statusCode != 200) {
      try {
        ...
        if (decodedError['errors'] != null && decodedError['errors'] is List && decodedError['errors'].isNotEmpty) {
          final errorMsg = decodedError['errors'][0]['detail'] ?? 'AbuseIPDB request failed.';
          throw Exception(errorMsg);
        }
      } catch (_) {
        // Fallback if parsing error response fails
      }
      throw Exception('AbuseIPDB API returned HTTP Status Code ${response.statusCode}');
    }
